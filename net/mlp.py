...
class MaskLayer(nn.Module):
    def __init__(self, z_dim, concept=7, z1_dim=4):
        super().__init__()
        self.z_dim = z_dim
        self.z1_dim = z1_dim
    ...
    def mix(self, z):
        ...
        if self.z1_dim == 1:
            ...
            if self.concept == 4:
                ...
                zy1, zy2, zy3, zy4, zy5, zy6, zy7 = torch.split(zy, self.z1_dim, dim=1)
        rx1 = self.net1(zy1)
        rx2 = self.net2(zy2)
        rx3 = self.net3(zy3)
        rx4 = self.net4(zy4)
...
class Decoder_DAG(nn.Module):
    ...
    def decode_sep(self, z, u, y=None):
        ...
        if self.z1_dim == 1:
            zy = zy.reshape(zy.size()[0], zy.size()[1], 1)
            if self.concept == 7:
                zy_split = zy.split(1, dim=1)
        else:
            if self.concept == 7:
                zy_split = torch.split(zy, self.z_dim // self.concept, dim=1)

        rx = [net(z_split) for net, z_split in zip(self.nets, zy_split)]
